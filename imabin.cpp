...
imadata<BYTE> imabin::imaunsignedchar(const bool etaldyn) const {
	int i,j,x;
	imadata<BYTE> ima_u(nblig,nbcol,1);
	for (i=0; i<nblig; i++)
		for (j=0; j<nbcol; j++) {
...
//	#else
//	for (i=0; i<nblig; i++)
//		for (j=0; j<nbcol; j++) if (*(bool*)Tsites[i*nbcol+j]) *(bool*)Tsites[i*nbcol+j]=0;
//	#endif
}
...
void imabin::add_impul_noise (float p) {
	int i,j;
	double a,b;
...
imabin imabin::dilate(eltstruct B) const {
	int i,j,i0,i2,j0,j2,ii,jj/*,nl2,nc2*/,iB;
	int nl=B.nl,nc=B.nc, x0=B.x0, y0=B.y0;
	bool z, id;
	imabin imaRes(*this);
...
imabin imabin::erode(eltstruct B) const {
	int i,j,i0,i2,j0,j2,ii,jj,iB;
	int nl=B.nl,nc=B.nc, x0=B.x0, y0=B.y0;
	bool z;
	imabin imaRes(*this);
...
imabin imabin::PGCC_CompoConnexes_MM (int iconnex, bool iaf) {
	int ncc, i, j, k, nblig=nlig(), nbcol=ncol(), nmax=0, kmax=-1;
	imadata<int> imaCC=CompoConnexes_MM(ncc,iconnex,iaf);
	int *Tnbpix=new int[ncc+1];
	for (k=0; k<=ncc; k++) Tnbpix[k]=0;
	for (i=0; i<nblig; i++)
		for (j=0; j<nbcol; j++) {k=imaCC(i,j); if (k>=0 && k<=ncc) Tnbpix[k]++; }
	for (k=1; k<=ncc; k++) if (Tnbpix[k]>nmax) {nmax=Tnbpix[k]; kmax=k;}
	if (Tnbpix!=NULL) delete[] Tnbpix;
	imabin imaCCmax(nblig,nbcol);
	for (i=0; i<nblig; i++)
		for (j=0; j<nbcol; j++) imaCCmax(i,j)=(imaCC(i,j)==kmax?1:0);
	return imaCCmax;
}
...
/* ---------------------------------------------------------------------*/ {
	...
	if (iaffich) cout<<" image de la transformee de Hough : "<<imaHough.nlig()<<" lignes et "<<imaHough.ncol()<<" colonnes\n";
...
imadata<int> imabin::transformee_Hough_cercles(const int N_Rho) {
	const int yc=nbcol/2, xc=nblig/2;
	int		i,j,iRho,ii,jj,i0,i2,j0,j2;
	float	xRho;
...
//				for (ii=0; ii<nblig; ii++)
//					for (jj=0; jj<nbcol; jj++) {
				i0=maxi(0,i-N_Rho); i2=mini(i+N_Rho,nblig-1);
				j0=maxi(0,j-N_Rho); j2=mini(j+N_Rho,nbcol-1);
				for (ii=i0; ii<=i2; ++ii)
					for (jj=j0; jj<=j2; ++jj) {
//						xRho=pow(pow((float)ii-i,2.f)+pow((float)jj-j,2.f),0.5f);
						xRho=sqrt((float)(ii-i)*(float)(ii-i)+(float)(jj-j)*(float)(jj-j));
...
imabin imabin::projette_ima_geom_fixe(const double dx, const double dy, const double hfact, const double phi, 
																			const int nblig2, const int nbcol2) const { 
	...
	double x,y;
	int ii,jj,i,j,d=A.nlig();
	imabin imaproj(nblig2,nbcol2);
	for (i=0; i<nblig2; ++i) // boucle sur les lignes de l'image sur laquelle on recale
		for (j=0; j<nbcol2; ++j) { // boucle sur les colonnes de l'image sur laquelle on recale
			x=A(0,0)+A(1,0)*j+A(2,0)*i; if (d>5) x+=A(3,0)*pow((float)j,2)+A(4,0)*pow((float)i,2)+A(5,0)*j*i; // estimation de la coordonnee colonne dans l'image à recaler
			y=A(0,1)+A(1,1)*j+A(2,1)*i; if (d>5) y+=A(3,1)*pow((float)j,2)+A(4,1)*pow((float)i,2)+A(5,1)*j*i; // estimation de la coordonnee ligne dans l'image à recaler
// interpolation au plus proche voisin
			jj=around(x); ii=around(y);
			if (ii>=0 && ii<nblig && jj>=0 && jj<nbcol) imaproj(i,j)=(*this)(ii,jj);
		}
	return imaproj;
}
...
float imabin::trace_strip (float fTheta, float fRho, int w) {
	const int n_dx=10, yc=nblig/2, xc=nbcol/2;
	int i,j,ii,jj,w2,cmin,cmax,lmin,lmax,xx=0;
	float x,y;
	if (fabs(cos(fTheta))>0.5) {
		w2=(int)(w*fabs(cos(fTheta))/2.f);
		for (i=0; i<nblig; i++)
			for (j=0; j<n_dx; j++) { 
				y=-(i+(float)j/n_dx); x=(fRho-(y+yc)*sin(fTheta))/cos(fTheta)+xc; // y est la coord. lig. (orientée vers le bas) et x la coord. col.
				ii=mini(maxi(0,around(-y)),nblig-1); jj=mini(maxi(0,around(x)),nbcol-1); 
				cmin=maxi(0,jj-w2); cmax=mini(jj+w2,nbcol-1);
				for (jj=cmin; jj<=cmax; jj++) if ((*this)(ii,jj)==false) {(*this)(ii,jj)=true; ++xx;}
			}
	} else {
		w2=(int)(w*fabs(sin(fTheta))/2.f);
		for (j=0; j<nbcol; j++) {
			for (i=0; i<n_dx; i++) {
				x=j+(float)i/n_dx; y=(fRho-(x-xc)*cos(fTheta))/sin(fTheta)-yc; // x la coord. col. et y est la coord. lig. (orientée vers le bas)
				ii=mini(maxi(0,around(-y)),nblig-1); jj=mini(maxi(0,around(x)),nbcol-1);
				lmin=maxi(0,ii-w2); lmax=mini(ii+w2,nblig-1);
				for (ii=lmin; ii<=lmax; ii++) if ((*this)(ii,jj)==false) {(*this)(ii,jj)=true; ++xx;}
			}
		}
	}
	return (float)xx;
