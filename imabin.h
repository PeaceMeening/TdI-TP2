...
{	bool *Tima;
 ...
 public:
	...
	imadata<BYTE> imaunsignedchar (const bool=1) const;
...
/* morphologie mathematique binaire */
	...
	imabin PGCC_CompoConnexes_MM (int=4, bool=0);
...
/* trace formes geometriques basiques */
	imabin projette_ima_geom_fixe(const double, const double, const double, const double, const int, const int) const; 
	...
	float trace_strip (float, float, int);
};
